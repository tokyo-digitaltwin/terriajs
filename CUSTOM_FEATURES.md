...
| Trait         | Type    | Default | Description                                                                                                                                                                   |
| :------------ | :------ | :------ | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| timeMode | string  |         | WMSからデータを取得する時に、データ取得期間の設定有無を指定するパラメータ。<br>取得期間を設定する場合は“range”を記述する。取得期間を設定しない場合は“instant”を記述する。データカタログに設定がない場合は、取得期間は指定されない。|
| timeRangeTemplate | string |         | WMSからデータを取得する時に、データ取得期間を指定するパラメータ。<br>1日分のデータを取得する記述例：”{date}T00:00:00Z/{date}T23:59:59Z” |

### ApiTableCatalogItem に追加されたtraits
| Trait         | Type    | Default | Description                                                                                                                                                                   |
| :------------ | :------ | :------ | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| headers | Object[]  |         | API にリクエストを送る時に付与する HTTP ヘッダーを指定するためのパラメータ。<br>{name: ヘッダー名(string),  value:ヘッダー値(string)}形式のobjectの配列で記述する。|
